...
class Quad:
    ...
    def Fint_Ktan_nonlinearkinematics(self, element, material, u_elem):
        ...
        for gp_idx, gauss_point in enumerate(self.gauss_points):
            ...
            # compute deformation gradient and transform [F_11, F_12, F_21, F_22].T to 3D tensor (plane strain)
            F = B_tilde @ x_elem
            ...
            # compute B matrix used to get Green-Lagrange strain from nodal displacements
            B = (
                np.array(
                    [
                        [F[0, 0], 0, F[1, 0], 0],
            ...
            Ktan_elem_const = B.T @ D @ B
            Ktan_elem_inist = B_tilde.T @ S_tilde @ B_tilde
            Ktan_elem += (
                (Ktan_elem_const + Ktan_elem_inist) * gauss_point.w * self.T * detJ
            )
