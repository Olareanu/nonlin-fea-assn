...
from .post import gausspoints_to_nodes
...
class Model:
    ...
    def prepare(self):
        ...
        for element_idx, element_nodes in enumerate(self.connectivity):
            ...
            # prepare the array storing the history variables at the Gauss points based on the history_allocation property
            if hasattr(self.material[0], "history_allocation"):
                history_allocation = self.material[0].history_allocation
                num_gp_history = (
                    max([max(history_allocation[idx]) for idx in history_allocation])
                    + 1
                )
                history_old_array = np.zeros(
                    (len(self.parent_element.gauss_points), num_gp_history)
                )
                history_new_array = np.zeros(
                    (len(self.parent_element.gauss_points), num_gp_history)
                )
            else:
                if hasattr(self.parent_element, "gauss_points"):
                    history_old_array = np.zeros(
                        (len(self.parent_element.gauss_points), 0)
                    )
                    history_new_array = np.zeros(
                        (len(self.parent_element.gauss_points), 0)
                    )
                else:
                    history_old_array = None
                    history_new_array = None
            ...
            element = Element(
                ...
                history_old=history_old_array,
                history_new=history_new_array,
            )
        ...
        # prepare any of the boundary conditions
        if self.contact_bcs is not None:
            pass

        # prepare matrix M for postprocessing
        if self.parent_element.element_type in ("quad4", "quad8", "quad9"):
            self.M = gausspoints_to_nodes.compute_M(self)
