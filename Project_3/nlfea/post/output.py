...
from . import gausspoints_to_nodes
...
def write_vtk(result_dir: str, increment: int, model, nodal_values):
    ...
    # project the Gauss point results to the nodes for continuum elements
    if model.parent_element.element_type in ("quad4", "quad8", "quad9"):
        nodal_values_projected = gausspoints_to_nodes.project(model)
        nodal_values.update(nodal_values_projected)
