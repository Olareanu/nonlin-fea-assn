...
def apply(model, Ktan: np.ndarray, Fint: np.ndarray, u: np.ndarray):
    ...
    for spring_bc in model.spring_bcs:
        ...
        for node in nodes:
            ...
            # spring force acting in direction of displacement (external spring forces are added to internal force vector to avoid scaling by load factor variable λ)
            Fint[dof] += stiffness * u[dof]

            # additional system stiffness contribution due to linear spring
