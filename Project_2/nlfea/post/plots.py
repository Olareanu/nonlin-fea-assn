...
def equilibrium_path(monitoring: dict, analytical_solution=None):
    ...
    if not all(stability):
        ...
        for unstable_increment in unstable_increments:
            # determine start point for shading
            if unstable_increment > 0:
                if unstable_increment == unstable_increments[0]:
            ...
            # determine end point for shading
            if unstable_increment < len(stability) - 1:
                if unstable_increment == unstable_increments[-1]:
                    u_end = u[unstable_increment + 1]
                else:
                    u_end = (
                        u[unstable_increment]
            ...
            # draw shading for unstable region
...
def detK_displacement(monitoring: dict):
    ...
    # extract quantities (don't consider initial state of u since detK was not computed)
    u = np.abs(monitoring["u"][1:])
    detK = monitoring["detK"][1:]
    critical = monitoring["critical"][1:]
